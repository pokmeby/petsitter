...
public class BlogController {
	...
	//게시판 글 작성 화면
		...
		public String read(BlogVO vo,BlogLikeVO likeVO, HttpSession session, @ModelAttribute("scri") SearchCriteria scri, Model model) throws Exception{
			System.out.println("게시물 조회! ");
			int likeTotal = service.likeTotal(vo.getBlog_bno());
			
			likeVO.setLikeBlog_bno(vo.getBlog_bno());
			likeVO.setLikeBlog_email((String)session.getAttribute("member"));
			if(session.getAttribute("member") != null) {
			int result = service.likeRead(likeVO);
		...
		public int like(BlogLikeVO likevo, HttpSession session, Model model, SearchCriteria scri) throws Exception{
			System.out.println("bloglikeVO 에 들어온 값은 ? " + likevo.getLikeBlog_bno());
			String id = (String)session.getAttribute("member");
			likevo.setLikeBlog_email(id);
			System.out.println("bloglikeVO 에 들어온 이메일은? " + likevo.getLikeBlog_email());
			int result = service.likeRead(likevo);
		...
		public String likeBlog( BlogLikeVO vo,  HttpSession session, RedirectAttributes rttr,SearchCriteria scri) throws Exception{
			String id = (String)session.getAttribute("member");
			vo.setLikeBlog_email(id);
			int result = service.likeRead(vo);
			System.out.println("좋아요 후 result 값은?? "+ result);
			if(result != 0) {
				service.likeDelete(vo);				
			}else {
				service.likeAdd(vo);
			}
			System.out.println("라이크 VO 에 전달된 값은? " + vo.getLikeBlog_bno());
			System.out.println("라이크 VO 에 전달된 값은? " + vo.getLikeBlog_email());
